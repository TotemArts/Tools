...
class SvnLookToDiff(object):
	SIGN_CONVERT_MAP =\
	{
		"A":  "+", # file added
		"D":  "-", # file deleted
		"U":  "=", # file contents modified
		"_U": "=", # file properties modified
		"UU": "="  # file contents and properties modified
	}
	
	@classmethod
	def convert(self, changelist):
		result = []
		for line in changelist.split("\n"):
			sign, path = re.split(r"\s+", line, 1)
			result.append("{0} {1}".format(self.SIGN_CONVERT_MAP[sign], path))
		return "\n".join(result)

# Discord has a message length limit of 2000 characters, so we need to set some maximum string length limits
MAX_LENGTH_LOG = 400
MAX_LENGTH_CHANGELIST = 1500
...
reporootdir = "/home/svn/datadrive/repos/svn/"    # CHANGE THIS TO POINT TO YOUR REPOSITORY ROOT DIRECTORY

# Local file path of svnlook.exe, which is used to get the commit's author, log, and changelist
svnlook = "/usr/bin/svnlook"    # CHANGE THIS TO POINT TO YOUR SVNLOOK PATH
...
message = "**{0}:** Subversion revision {1} committed by {2}:\n```{3}``` \n```diff\n{4}```".format(reponame, revision, author, log, SvnLookToDiff.convert(changelist))
